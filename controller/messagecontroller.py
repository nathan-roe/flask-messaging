...
def send_message_request(cur_user, contact_user):
    cur_message_rels = get_related_message_groups_by_user(cur_user, contact_user)
...
def update_message_request(cur_user, owner, declined = False):
    ...
    if message_rel.status != Constants.MessageRelConstants.REQUESTED:
        raise ValueError("Message request has already been received.")
...
def send_message(cur_user, receiver, message):
    message_rel = get_related_message_groups_by_user(cur_user, receiver).first()
...
def get_related_message_groups_by_user(cur_user, contact):
...
def get_all_related_message_groups(cur_user):
    all_message_rels = UserMessageRelationship.query.filter(
        (UserMessageRelationship.contact == cur_user)
        | (UserMessageRelationship.owner == cur_user)
    )

    return all_message_rels
